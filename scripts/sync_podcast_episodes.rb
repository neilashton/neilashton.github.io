...
def body_html(overview:, links:, chapters:, transcript:, transcript_path:, transcript_corrected:, spotify_url:, youtube_video_id:, video_duration_display:)
  sections = []
  sections << <<~HTML.chomp
  ...
  if chapters.any?
    chapter_heading = video_duration_display ? "Audio chapters" : "Chapters"
    chapter_note = if video_duration_display
      %(\n  <p class="episode-chapters-note">\n    These timestamps follow the <a href="#{CGI.escapeHTML(spotify_url)}" rel="external noopener">Spotify/audio edition</a>, including the podcast intro. The <a href="https://www.youtube.com/watch?v=#{CGI.escapeHTML(youtube_video_id)}" rel="external noopener">YouTube edition</a> starts directly with the discussion, so its chapter timings differ.\n  </p>)
    else
      ""
    end
    ...
    sections << <<~HTML.chomp
      <section class="episode-section" aria-labelledby="episode-chapters">
        <h2 id="episode-chapters">#{chapter_heading}</h2>#{chapter_note}
...
episodes = rss_episodes.map do |episode|
  ...
  episode.merge(
    ...
    "video_duration_display" => youtube_episode["video_duration_display"],
...
episodes.each_with_index do |episode, index|
  ...
  front_matter = {
    ...
    "video_duration_display" => episode["video_duration_display"],
    "video_duration_iso" => episode["video_duration_display"] && duration_iso(episode["video_duration_display"]),
  ...
  body = body_html(
    ...
    transcript_corrected: episode.fetch("transcript_corrected"),
    spotify_url: episode.fetch("spotify_url"),
    youtube_video_id: youtube_episodes.fetch("s#{episode.fetch("season")}-e#{episode.fetch("episode")}").fetch("id"),
    video_duration_display: episode["video_duration_display"],
  )
